...
# Remove linux-headers if they exist (may not be present with Proxmox kernel)
apt-get remove -y linux-headers-$(uname -r) build-essential make 2>/dev/null || apt-get remove -y build-essential make 2>/dev/null || true
apt-get purge -y $(dpkg --list | grep '^rc' | awk '{print $2}') 2>/dev/null || true

# Remove any remaining non-Proxmox kernel from installation (~400MB)
# Be careful not to remove the Proxmox kernel (pve)
KERNELS_TO_REMOVE=$(dpkg -l | egrep 'linux-image-[0-9]' | grep -v $(uname -r) | grep -v pve | awk '{ print $2 }')
if [[ -n "$KERNELS_TO_REMOVE" ]]; then
    apt-get purge -y $KERNELS_TO_REMOVE || true
fi
...
# Disable cloud-init for firstboot (let zproxmox-init.sh handle it)
